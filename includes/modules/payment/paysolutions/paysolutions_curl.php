<?php
/**
 * paysolutions_curl.php communications class for paysolutions Express Checkout / Website Payments Pro / Payflow Pro payment methods
 *
 * @package paymentMethod
 * @copyright Copyright 2003-2006 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: paysolutions_curl.php 5425 2006-12-28 12:50:46Z drbyte $
 */

/**
 * paysolutions NVP (v2.3) and Payflow Pro (v4 HTTP API) implementation via cURL.
 */
class paysolutions_curl extends base {
   ...
   */
  var $_endpoints = array('live'    => 'https://api-3t.paysolutions.com/nvp',
                          'sandbox' => 'https://api.sandbox.paysolutions.com/nvp');
  /**
   ...
   */
  function paysolutions_curl($params = array()) {
   ...
   * SetExpressCheckout
   *
   * Prepares to send customer to paysolutions site so they can 
   * log in and choose their funding source and shipping address.
   * 
   * The token returned to this function is passed to paysolutions in 
   * order to link their paysolutions selections to their cart actions.
   */
  function SetExpressCheckout($amount, $returnUrl, $cancelUrl, $optional = array()) {
    ...
    if (defined('MODULE_PAYMENT_paysolutionsWPP_PAGE_STYLE'))   $values['PAGESTYLE'] = MODULE_PAYMENT_paysolutionsWPP_PAGE_STYLE;
    if (defined('MODULE_PAYMENT_paysolutionsWPP_HEADER_IMAGE')) $values['HDRIMG'] = urlencode(MODULE_PAYMENT_paysolutionsWPP_HEADER_IMAGE);
    if (defined('MODULE_PAYMENT_paysolutionsWPP_PAGECOLOR'))    $values['PAYFLOWCOLOR'] = MODULE_PAYMENT_paysolutionsWPP_PAGECOLOR;
    if (defined('MODULE_PAYMENT_paysolutionsWPP_HEADER_BORDER_COLOR')) $values['HDRBORDERCOLOR'] = MODULE_PAYMENT_paysolutionsWPP_HEADER_BORDER_COLOR;
    if (defined('MODULE_PAYMENT_paysolutionsWPP_HEADER_BACK_COLOR')) $values['HDRBACKCOLOR'] = MODULE_PAYMENT_paysolutionsWPP_HEADER_BACK_COLOR;
   ...
   *
   * When customer returns from paysolutions site, this retrieves their payment/shipping data for use in Zen Cart
   */
   ...
   *
   * Completes the sale using paysolutions as payment choice
   */
   ...
   *
   * Used to read data from paysolutions for a given transaction
   */
...
// extra debug email: ///
    if (MODULE_PAYMENT_paysolutionsWPP_DEBUGGING == 'Log and Email') {
      ...
      zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, 'paysolutions Debug log - ' . $operation, $message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
    }
/////////////////////////
  ...
  function log($message, $token = '') {
    ...
    if ($token == '') $token = $_SESSION['paysolutions_ec_token'];
    if ($token == '') $token = time();
    $token .= $tokenHash;
    $file = $this->_logDir . '/' . 'paysolutions_Debug_' . $token . '.log';
    $fp = @fopen($file, 'a');
    @fwrite($fp, $message . "\n\n");
    @fclose($fp);
  }
