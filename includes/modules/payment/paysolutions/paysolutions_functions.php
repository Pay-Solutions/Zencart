<?php
/**
 * functions used by payment module class for paysolutions IPN payment method
 *
 ...
 * @version $Id: paysolutions_functions.php 5427 2006-12-28 14:59:24Z drbyte $
 */

// Functions for paysolutions processing
  function datetime_to_sql_format($paysolutionsDateTime) {
    //Copyright (c) 2004 DevosC.com
    $months = array('Jan' => '01', 'Feb' => '02', 'Mar' => '03', 'Apr' => '04', 'May' => '05',  'Jun' => '06',  'Jul' => '07', 'Aug' => '08', 'Sep' => '09', 'Oct' => '10', 'Nov' => '11', 'Dec' => '12');
    $hour = substr($paysolutionsDateTime, 0, 2);$minute = substr($paysolutionsDateTime, 3, 2);$second = substr($paysolutionsDateTime, 6, 2);
    $month = $months[substr($paysolutionsDateTime, 9, 3)];
    $day = (strlen($day = preg_replace("/,/" , '' , substr($paysolutionsDateTime, 13, 2))) < 2) ? '0'.$day: $day;
    $year = substr($paysolutionsDateTime, -8, 4);
    if (strlen($day)<2) $day = '0'.$day;
    return ($year . "-" . $month . "-" . $day . " " . $hour . ":" . $minute . ":" . $second);
  }

  function ipn_debug_email($message, $email_address = '', $always_send = false) {
    static $paysolutions_error_counter;
    static $paysolutions_instance_id;
    if ($email_address == '') $email_address = (defined('MODULE_PAYMENT_paysolutions_DEBUG_EMAIL_ADDRESS') ? MODULE_PAYMENT_paysolutions_DEBUG_EMAIL_ADDRESS : STORE_OWNER_EMAIL_ADDRESS);
    if(!isset($paysolutions_error_counter)) $paysolutions_error_counter = 0;
    if(!isset($paysolutions_instance_id)) $paysolutions_instance_id = time() . '_' . zen_create_random_value(4);
    if (MODULE_PAYMENT_paysolutionsWPP_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_paysolutions_IPN_DEBUG == 'Log and Email' || $always_send) {
      $paysolutions_error_counter ++;
//      mail($email_address,'IPN DEBUG message (' . $paysolutions_instance_id . ') #' . $paysolutions_error_counter, $message);
      zen_mail(STORE_OWNER, $email_address, 'IPN DEBUG message (' . $paysolutions_instance_id . ') #' . $paysolutions_error_counter, $message, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
    }
    if (MODULE_PAYMENT_paysolutions_IPN_DEBUG == 'Log and Email' || MODULE_PAYMENT_paysolutions_IPN_DEBUG == 'Log File' || MODULE_PAYMENT_paysolutions_IPN_DEBUG == 'Yes' || MODULE_PAYMENT_paysolutionsWPP_DEBUGGING == 'Log File' || MODULE_PAYMENT_paysolutionsWPP_DEBUGGING =='Log and Email') ipn_add_error_log($message, $paysolutions_instance_id);
  }
  function ipn_get_stored_session($session_stuff) {
    global $db;
    if (!is_array($session_stuff)) {
      ipn_debug_email('IPN FATAL ERROR::Could not find custom variable in post, cannot re-create session as paysolutions IPN transaction.');
      return false;
    }
    $sql = "SELECT *
            FROM " . TABLE_paysolutions_SESSION . "
            WHERE session_id = :sessionID";
    $sql = $db->bindVars($sql, ':sessionID', $session_stuff[1], 'string');
    $stored_session = $db->Execute($sql);
    if ($stored_session->recordCount() < 1) {
      ipn_debug_email('IPN FATAL ERROR::Could not find stored session in DB, cannot re-create session as paysolutions IPN transaction.');
      return false;
    }
 ...
 */
  function ipn_lookup_transaction($postArray) {
    ...
    $useTable = TABLE_paysolutions;
    if (MODULE_PAYMENT_paysolutions_TESTING == 'Test') $useTable = TABLE_paysolutions_TESTING;
    $ordersID = 0;
    $paysolutionsipnID = 0;
    $transType = 'unknown';

    $sql = "SELECT zen_order_id, paysolutions_ipn_id, payment_status, txn_type, pending_reason
                FROM " . $useTable . "
    ...
    if (isset($postArray['parent_txn_id'])) {
      ...
      if($ipn_id->RecordCount() > 0) {
        ...
        $paysolutionsipnID = $ipn_id->fields['paysolutions_ipn_id'];
      }
    } else {
    ...
    return array('zen_order_id' => $ordersID, 'paysolutions_ipn_id' => $paysolutionsipnID, 'txn_type' => $transType);
  }
/**
 * IPN Validation
 ...
 */
  function ipn_validate_transaction($info, $postArray) {
    ...
    if (strtolower($postArray['business']) != strtolower(MODULE_PAYMENT_paysolutions_BUSINESS_ID) && strtolower($postArray['receiver_email']) != strtolower(MODULE_PAYMENT_paysolutions_BUSINESS_ID)) {
      ipn_debug_email('IPN WARNING::Transaction email address NOT matched.' . "\n" . 'From IPN = ' . $postArray['business'] . ' | ' . $postArray['receiver_email'] . "\n" . 'From CONFIG = ' .  MODULE_PAYMENT_paysolutions_BUSINESS_ID);
      return false;
    }
    return true;
  }
  ...
  function valid_payment($info, $amount, $currency) {
    global $currencies;
    if (MODULE_PAYMENT_paysolutions_CURRENCY == 'Selected Currency') {
      $my_currency = $_SESSION['currency'];
    } else {
      $my_currency = substr(MODULE_PAYMENT_paysolutions_CURRENCY, 5);
    }
    ...
    if ( ($_POST['mc_currency'] != $my_currency) || ($_POST['mc_gross'] != $transaction_amount && $_POST['mc_gross'] != -0.01) && MODULE_PAYMENT_paysolutions_TESTING != 'Test' ) {
      ipn_debug_email('IPN WARNING::Currency/Amount Mismatch.  Details: ' . "\n" . 'paysolutions email address = ' . $_POST['business'] . "\n" . ' | mc_currency = ' . $_POST['mc_currency'] . "\n" . ' | submitted_currency = ' . $my_currency . "\n" . ' | order_currency = ' . $currency . "\n" . ' | mc_gross = ' . $_POST['mc_gross'] . "\n" . ' | converted_amount = ' . $transaction_amount . "\n" . ' | order_amount = ' . $amount );
      return false;
    }
    return true;
  }
 ...
 *    (1) we find a matching record in the "paysolutions" table
...
// 2. could be an express-checkout "pending" transaction which has been Accepted in the merchant's paysolutions console and needs activation in Zen Cart
 ...
 */
  function ipn_create_order_array($new_order_id, $txn_type) {
    $paysolutions_order = array('zen_order_id' => $new_order_id,
                          'txn_type' => $txn_type,
 ...
 */
  function ipn_create_order_history_array($insert_id) {
    ...
    return $paysolutions_order_history;
  }
/**
 * Create order-update from IPN data
 */
  function ipn_create_order_update_array($txn_type) {
    $paysolutions_order = array('reason_code' => $_POST['reason_code'],
                          'payment_type' => $_POST['payment_type'],
                          'txn_type' => $txn_type,
                          'parent_txn_id' => $_POST['parent_txn_id'],
                          'payment_status' => $_POST['payment_status'],
 ...
 */
  function ipn_simulate_ipn_handler($count) {
    global $db;
    $sql = "select * from " . TABLE_paysolutions_TESTING . " order by paysolutions_ipn_id desc limit " . (int)$count;
    $paysolutions_testing = $db->execute($sql);
    while (!$paysolutions_testing->EOF) {
      $paysolutions_fields[] = $paysolutions_testing->fields;
      $paysolutions_testing->moveNext();
    }
    $paysolutions_fields = array_reverse($paysolutions_fields);
    foreach ($paysolutions_fields as $value) {
 ...
 * Debug logging
 */
  function ipn_add_error_log($message, $paysolutions_instance_id = '') {
    if ($paysolutions_instance_id == '') $paysolutions_instance_id = date('mdYGi');
    $fp = @fopen('includes/modules/payment/paysolutions/logs/ipn_' . $paysolutions_instance_id . '.log', 'a');
    @fwrite($fp, date('M d Y G:i') . ' -- ' . $message . "\n\n");
    @fclose($fp);
    // if (MODULE_PAYMENT_paysolutions_TESTING == 'Test') echo date('d M Y G:i') . ' -- ' . $message . "\n";
  }
/**
 * Debug to file
 */
...
/**
 * Verify IPN by sending it back to paysolutions for confirmation
 */
